...
public class onezztwo {
    ...
    public List<String> commonChars(String[] words) {
        ...
        // Initialize the charCount array to hold the maximum possible value
        int[] charCount = new int[26];
        Arrays.fill(charCount, Integer.MAX_VALUE);

        // Iterate over each word
        for (String word : words) {
            // Initialize a temporary count array for the current word
            int[] wordCharCount = new int[26];
            for (char ch : word.toCharArray()) {
                wordCharCount[ch - 'a']++;
            }

            // Update the global charCount array to the minimum counts
            for (int i = 0; i < 26; i++) {
                charCount[i] = Math.min(charCount[i], wordCharCount[i]);
            }
        }

        // Construct the result list from the charCount array
        for (int i = 0; i < 26; i++) {
            char ch = (char) ('a' + i);
            if (charCount[i] > 0) {
